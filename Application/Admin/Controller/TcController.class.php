...
//Excel导入
    ...
    public function excel_runexport(){
        ...
        $data= M('tc')->where($map)->order("id")->select();
        ...
        foreach($data as $k => $v){
            ...
            if($id==$rid){
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('C'.$num, M('tc')->where("id=$id")->sum('score'))
                    ->setCellValue('D'.$num, $v['title'])
                    ->setCellValue('E'.$num, $v['score']);
                $rid = $v['id'];
            }
    ...
    public function excel_runimport1(){
        import("Org.Util.PHPExcel");
        $PHPExcel=new \PHPExcel();
        import("Org.Util.PHPExcel.Reader.Excel5");
        ...
        if (! empty ( $_FILES ['file_stu'] ['name'] ))
        {
            ...
            */
            ...
            foreach ( $res as $k => $v )
            {
                ...
                {
                    $data ['tid'] = $v [0];
                    $classify = $data ['classify'] = $v [1];
                    $id = $data ['id'] = $v [2];
                    $data ['score'] = $v [3];
                    $data ['name'] = M('student')->where("id=$id")->getField('name');
                    $data ['title'] = M('fwsc') -> where("id=$classify")->getField('title');
//                    $data ['id'] = $v[0];
//                    $data['title'] = $v[1];
//                    $data ['time'] = $v[2];
